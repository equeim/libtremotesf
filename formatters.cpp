...
fmt::format_context::iterator fmt::formatter<QString>::format(const QString& string, fmt::format_context& ctx) FORMAT_CONST {
    return fmt::formatter<string_view>::format(toFmtStringView(string.toUtf8()), ctx);
}

fmt::format_context::iterator fmt::formatter<QStringView>::format(const QStringView& string, fmt::format_context& ctx) FORMAT_CONST {
    return fmt::formatter<string_view>::format(toFmtStringView(string.toUtf8()), ctx);
}

fmt::format_context::iterator fmt::formatter<QLatin1String>::format(const QLatin1String& string, fmt::format_context& ctx) FORMAT_CONST {
    return fmt::formatter<string_view>::format(std::string_view(string.data(), static_cast<size_t>(string.size())), ctx);
}

fmt::format_context::iterator fmt::formatter<QByteArray>::format(const QByteArray& array, fmt::format_context & ctx) FORMAT_CONST {
    return fmt::formatter<string_view>::format(toFmtStringView(array), ctx);
}

#if QT_VERSION_MAJOR >= 6

fmt::format_context::iterator fmt::formatter<QUtf8StringView>::format(const QUtf8StringView& string, format_context& ctx) FORMAT_CONST {
    return fmt::formatter<string_view>::format(string_view(string.data(), static_cast<size_t>(string.size())), ctx);
}

fmt::format_context::iterator fmt::formatter<QAnyStringView>::format(const QAnyStringView& string, format_context& ctx) FORMAT_CONST {
...
namespace {
    fmt::format_context::iterator formatSystemError(std::string_view type, const std::system_error& e, fmt::format_context& ctx) {
        const int code = e.code().value();
        return fmt::format_to(ctx.out(), "{}: {} (error code {} ({:#x}))", type, e.what(), code, static_cast<unsigned int>(code));
    }
}

fmt::format_context::iterator fmt::formatter<std::exception>::format(const std::exception& e, fmt::format_context& ctx) FORMAT_CONST {
...
fmt::format_context::iterator fmt::formatter<std::system_error>::format(const std::system_error& e, fmt::format_context& ctx) FORMAT_CONST {
    return formatSystemError(libtremotesf::typeName(e), e, ctx);
}

#ifdef Q_OS_WIN

fmt::format_context::iterator fmt::formatter<winrt::hstring>::format(const winrt::hstring& str, fmt::format_context& ctx) FORMAT_CONST {
...
fmt::format_context::iterator fmt::formatter<winrt::hresult_error>::format(const winrt::hresult_error& e, fmt::format_context& ctx) FORMAT_CONST {
